...
# Plots Cutoff lines
plt.axhline(y=y_data[1], color='red', linestyle='-', label="Zero Cutoff", zorder=1)
plt.axhline(y=y_data[0], color='red', linestyle='--', label="Zero Nominal", zorder=1)
plt.axhline(y=y_data[3], color='cyan', linestyle='-', label="One Cutoff", zorder=1)
plt.axhline(y=y_data[2], color='cyan', linestyle='--', label="One Nominal", zorder=1)


# Plots all readings as a line and as a scatter plot
plt.plot(y_data[6:], color="black", linestyle='-', label="MemFree plot", zorder=2)
plt.scatter(range(len(y_data)-6), y_data[6:], color="black", marker='.', label="MemFree values", zorder=2)
...
plt.title("MemFree Values Recorded by Sink\nhold time = " + str(y_data[4]) + "us; channel time = " + str(y_data[5]) + "s") 
