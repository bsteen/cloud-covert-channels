...
char received_data[BYTES_TO_TRANSFER];     // Data received by the sender is stored in this array; CHANGE THIS TO AN OUTPUT FILE LATER
...
// Locks or unclocks the ACK_file
    ...
    do{
        ...
        if(get_lock_value(transmit_lock) && !value_read){   // If transmit lock is set and locks haven't already been read
            ...
            for(i = 0; i < NUM_DATA_LOCKS; i++){
                received_data[byte_number] |= get_lock_value(data_locks[i]) << i;
            }

            printf("0x%02x\n", received_data[byte_number] & 0xFF);
